...
def list_resource_manager_apis(root_dir):
    ...
    for dirpath, dirnames, filenames in os.walk(root_dir):
        ...
        # Check the folder
        # ./submodules/azure-rest-api-specs/
        #   specification/<folder>/resource-manager/
        #   <PROVIDER>/stable or preview/<VERSION>/
        if len(parts) == PATH_DEPTH and parts[-4] == "resource-manager":
            provider = {}
            if re.match(r"^Microsoft\.*", parts[-3]) and (
                ...
                parts.pop()
                parts.pop()
                provider["path"] = os.sep.join(parts)
                print(f"path = {provider['path']}")

                if provider["provider_name"] not in resource_manager_api_dict:
                    resource_manager_api_dict[provider["provider_name"]] = {
                        "path": provider["path"],
                        "api_versions": [],
                    }
                resource_manager_api_dict[provider["provider_name"]][
        ...
        if len(parts) == (PATH_DEPTH + 1) and parts[-5] == "resource-manager":
            provider = {}
    ...
    resource_manager_api_list = [
        {"provider_name": k, "properties": v}
        for k, v in resource_manager_api_dict.items()
    ]
    # Sort by provider, then api_version
    resource_manager_api_list.sort(key=lambda x: (x["provider_name"]))
